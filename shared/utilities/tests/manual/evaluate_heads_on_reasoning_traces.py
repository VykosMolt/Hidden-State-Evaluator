"""Evaluate HH-trained and code-trained registry heads on generated reasoning traces."""
...
FEATURES_PT = REPORT_DIR / "reasoning_trace_features_2026-05-17.pt"
HEADS_PT = REPORT_DIR / "bg_head_registry_2026-05-17.pt"
OUTPUT_JSON = REPORT_DIR / "reasoning_trace_transfer_2026-05-17.json"
OUTPUT_MD = REPORT_DIR / "reasoning_trace_transfer_2026-05-17.md"
...
def specialist_verdict(best_hh: dict[str, Any] | None, best_code: dict[str, Any] | None, best_overall: dict[str, Any] | None, baseline: float, n_records: int) -> str:
    if n_records < 10 or not best_overall or not best_hh or not best_code:
        return "INSUFFICIENT"
    hh = best_hh["metrics"]
...
def write_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        f"REASONING_TRACE_TRANSFER_VERDICT = {payload['reasoning_trace_transfer_verdict']}",
        f"REASONING_TRACE_SPECIALIST_VERDICT = {payload['reasoning_trace_specialist_verdict']}",
        "",
...
def main() -> None:
    ...
    baseline = random_top1(records)
    rows = evaluate_records(records, heads, device, "reasoning_traces")
    ...
    payload = {
        "reasoning_trace_transfer_verdict": transfer_verdict(best_overall, baseline),
        "reasoning_trace_specialist_verdict": specialist_verdict(best_hh, best_code, best_overall, baseline, len(records)),
        "summary": {
            "REASONING_TRACE_TRANSFER_VERDICT": transfer_verdict(best_overall, baseline),
            "REASONING_TRACE_SPECIALIST_VERDICT": specialist_verdict(best_hh, best_code, best_overall, baseline, len(records)),
    ...
    print(f"REASONING_TRACE_TRANSFER_VERDICT = {payload['reasoning_trace_transfer_verdict']}")
    print(f"REASONING_TRACE_SPECIALIST_VERDICT = {payload['reasoning_trace_specialist_verdict']}")
