...
cd "$(dirname "$0")/../../.."
PY=./venv/bin/python
OUT=opi/branching/wellformed_terminal_v4_20260727
LORA_LOG=opi/control/lora_s3a_pilot_20260727/run.log
LOG=$OUT/run.log
...
$PY -u shared/utilities/tests/manual/bg_v4_wellformed_generate.py --task-offset 1350 --n-tasks 150 --tag ext1 >> "$LOG" 2>&1 || { say "FAIL wf ext1 generate"; exit 1; }
say "START wf pooled selection analysis"
$PY -u shared/utilities/tests/manual/bg_v4_wellformed_selection.py >> "$LOG" 2>&1 || { say "FAIL wf pooled selection"; exit 1; }
