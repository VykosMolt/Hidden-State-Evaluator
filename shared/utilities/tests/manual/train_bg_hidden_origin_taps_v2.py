"""Retrain tiny hidden-origin branch taps on the v2 dataset."""
...
OUT_PT = HEADS_V2_PT
OUT_JSON = V2_ROOT / "training_log_v2.json"
OUT_MD = V2_ROOT / "training_report_v2.md"
...
def verdict_for(dataset_verdict: str, rows: Sequence[dict[str, Any]], train_pairs: int, val_pairs: int) -> str:
    if train_pairs < 2 or val_pairs < 1:
        return "INSUFFICIENT"
    if train_pairs < 60 or val_pairs < 10 or dataset_verdict == "DATA_LIMITED":
        floor = "DATA_LIMITED"
    else:
        floor = ""
    valid = [
...
def main() -> int:
    ...
    ensure_v2_root()
    started = time.time()
    if not DATASET_V2_PT.exists():
        payload = {"BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT": "INSUFFICIENT", "blocker": "missing dataset v2"}
        write_json(OUT_JSON, payload)
        write_md(OUT_MD, ["# Hidden-Origin Tap Training V2", "", "BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT = INSUFFICIENT"])
        print("BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT = INSUFFICIENT", flush=True)
        return 1
    ...
    for variant in variant_names:
        ...
        for config in CONFIGS:
            ...
            for architecture in ARCHITECTURES:
                for seed in seeds:
                    for lr in lrs:
                        print(f"training v2 {variant} {architecture} {config} seed={seed} lr={lr}", flush=True)
                        ...
                        row = {
                            "head_group": "hidden_origin_branch_taps_v2",
                            "variant": variant,
                            "architecture": architecture,
                            "config": config,
    ...
    payload = {
        "BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT": verdict,
        "verdict": verdict,
    ...
    lines = [
        "# Hidden-Origin Tap Training V2",
        "",
        f"BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT = {verdict}",
        "",
        ...
        "",
        "The headline result remains the old-style exact antisymmetric tiny heads. Diagnostic variants are marked by `variant` and do not replace the primary safe deterministic headline.",
        "",
        "## Config Summary",
        "",
    ]
    ...
    print(f"BG_HIDDEN_ORIGIN_TAP_TRAINING_V2_VERDICT = {verdict}", flush=True)
