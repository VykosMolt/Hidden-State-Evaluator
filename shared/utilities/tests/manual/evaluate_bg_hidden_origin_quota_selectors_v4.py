"""Evaluate hidden-origin selectors on v4 quota-heldout data."""
...
OUT_JSON = HEADS_V4_PT.parent / "heldout_eval_v4.json"
OUT_MD = HEADS_V4_PT.parent / "heldout_eval_v4.md"
OUT_CSV = HEADS_V4_PT.parent / "heldout_eval_v4_rows.csv"


def best_heads_by_config(heads: Sequence[dict[str, Any]], variant: str = "v4_only_primary_safe") -> list[dict[str, Any]]:
...
def support_ready(heldout_tasks: int, diverse_groups: int, heldout_pairs: int) -> bool:
    return (
        heldout_tasks >= PRIMARY_MINIMUMS["heldout"]["task_ids"]
        and diverse_groups >= PRIMARY_MINIMUMS["heldout"]["behaviorally_diverse_groups"]
        and heldout_pairs >= PRIMARY_MINIMUMS["heldout"]["non_tie_pairs"]
    )
...
def verdict_for(ready: bool, behavior_metrics: dict[str, Any]) -> tuple[str, str]:
    ...
    if best_name == "v4_hidden_origin_tap" and best_top1 > random_top1 and best_top1 > clean_top1 and best_top1 >= old_top1:
...
def main() -> int:
    ...
    if not DATASET_V4_PT.exists():
        payload = {"BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT": "INSUFFICIENT", "verdict": "INSUFFICIENT", "blocker": "missing v4 dataset"}
        write_json(OUT_JSON, payload)
        write_md(OUT_MD, ["# Hidden-Origin Selector Evaluation V4", "", "BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT = INSUFFICIENT"])
        print("BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT = INSUFFICIENT", flush=True)
        return 1
    dataset = torch.load(DATASET_V4_PT, map_location="cpu", weights_only=False)
    trained = torch.load(HEADS_V4_PT, map_location="cpu", weights_only=False) if HEADS_V4_PT.exists() else {"verdict": "INSUFFICIENT", "heads": []}
    ...
    all_rows = load_v4_branch_rows()
    heldout_primary_rows = [row for row in all_rows if str(row.get("task_id")) in test_tasks and primary_safe_v4_row(row)]
    ...
    v4_heads = [row for row in list(trained.get("heads") or []) if row.get("variant") == "v4_only_primary_safe" and row.get("flip_diagnostics", {}).get("passes")]
    heads = {
        ...
        "v4": max(v4_heads, key=lambda row: float(row["metrics"].get("validation_pairwise_accuracy", -1.0))) if v4_heads else None,
    }
    eval_rows = []
    ...
    pairwise_by_head.extend(pairwise_rows(best_heads_by_config(list(trained.get("heads") or []), "v4_only_primary_safe"), test_pairs, device, "v4"))
    ...
    payload = {
        "BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT": verdict,
        "verdict": verdict,
        "HIDDEN_ORIGIN_SELECTOR_BEST_AVAILABLE_V4_EVAL": best_selector,
        "dataset": rel(DATASET_V4_PT),
        "heads": rel(HEADS_V4_PT),
    ...
    lines = [
        "# Hidden-Origin Selector Evaluation V4",
        "",
        f"BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT = {verdict}",
        "",
    ...
    lines.extend(md_table(metric_rows[:220], ["policy", "config", "architecture", "groups", "top1", "reward", "regret", "top2_oracle"]))
    lines.extend(["", "## Heldout Pairwise", ""])
    lines.extend(md_table([{**row, "pairwise_accuracy": rate(row["pairwise_accuracy"]), "validation_pairwise_accuracy": rate(row["validation_pairwise_accuracy"])} for row in pairwise_by_head[:120]], ["source", "head_id", "heldout_pairs", "config", "architecture", "pairwise_accuracy", "validation_pairwise_accuracy"]))
    write_md(OUT_MD, lines)
    print(f"BG_HIDDEN_ORIGIN_SELECTOR_EVAL_V4_VERDICT = {verdict}", flush=True)
