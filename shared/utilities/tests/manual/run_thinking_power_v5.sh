...
cd "$(dirname "$0")/../../.."
PY=./venv/bin/python
OUT=opi/preanswer/thinking_preanswer_power_v5_20260727
LOG=$OUT/run.log
mkdir -p "$OUT"
...
$PY -u shared/utilities/tests/manual/bg_v5_thinking_power_generate.py >> "$LOG" 2>&1 \
    || { say "FAIL v5 thinking power generate"; exit 1; }
say "DONE v5 thinking power generate"

say "START v5 thinking power analysis"
$PY -u shared/utilities/tests/manual/bg_v5_thinking_power_analysis.py >> "$LOG" 2>&1 \
