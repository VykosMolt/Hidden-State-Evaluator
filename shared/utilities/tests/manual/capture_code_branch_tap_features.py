"""Capture pooled Ouro-RLTT tap features for code branch candidates."""
...
try:
    from utilities.tests.manual.code_branch_pilot_lib import REPORT_DIR, RLTT_MODEL_PATH, output_path, repo_path, write_json
...
DEFAULT_INPUT = REPORT_DIR / "code_branch_tournaments_2026-05-16.json"
DEFAULT_OUTPUT = REPORT_DIR / "code_branch_tap_features_2026-05-16.pt"
DEFAULT_MD = REPORT_DIR / "code_branch_tap_features_2026-05-16.md"
...
def candidate_list(tournament: dict[str, Any], primary: str) -> list[dict[str, Any]]:
    ...
    return list(tournament.get("diagnostic_candidates", []))


def write_md(path: Path, result: dict[str, Any]) -> None:
    lines = [
        "# Code Branch Tap Feature Capture",
        "",
        ...
        "",
        "Feature texts contain the problem prompt and candidate code only. Unit-test labels/results are not included in the model input.",
        "",
    ]
...
def main() -> None:
    ...
    verdict = payload.get("code_tournament_verdict")
    if verdict not in {"CLEAN", "DIAGNOSTIC_ONLY"}:
        raise SystemExit(f"CODE_TOURNAMENT_VERDICT={verdict}; feature capture blocked")
    primary = payload.get("primary_eval_set")
    tournaments = [
        t for t in payload.get("tournaments", [])
        if (primary == "strict_clean" and t.get("strict_clean")) or (primary == "diagnostic_mixed" and t.get("diagnostic_mixed"))
    ]
    texts: list[str] = []
    spans: list[tuple[int, int]] = []
    for tournament in tournaments:
        start = len(texts)
        for cand in candidate_list(tournament, str(primary)):
    ...
    for tournament, (start, end) in zip(tournaments, spans):
        candidates = candidate_list(tournament, str(primary))
