...
The DualAnchor science/reasoning repair v2 run adds the current pre-steering domain
scope:
...
Reasoning is the headline scope under confidence-gated top1 else top5/full survivor-set
handoff. Science remains diagnostic/excluded from headline steering because regenerated
science heldout stayed branch-generation weak and robust parser promotion was not
validated. This does not alter the Phase 1 routing lock below. No steering, production
routing change, action steering, autoregressive fork/carry claim, or compute-savings
claim follows from the v2 repair run.

Report: `../../bg_dualanchor_science_reasoning_repair_v2.md`.

## 2026-05-31 Supersession Note

This document remains the Phase 1 read-only controller/routing lock. It is no longer the active frontier for Phase 2 branch selection.

The active Phase 2a branch-selection baseline candidate is documented in:
...
This does not alter the Phase 1 routing lock below. No production routing change, action steering, autoregressive fork/carry claim, or compute-savings claim follows from the DualAnchor v3 probe.
