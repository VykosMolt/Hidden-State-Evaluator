# Evaluator Domain Transfer Notes

## Layer 24/36 bipartite + Thinking-vs-RLTT (2026-05-14)

Source probes:
- `utilities/evaluator/probes/probe_bipartite_layers_24_36.py`
- `utilities/evaluator/probes/probe_cross_backbone_layers.py`

Inputs: 200 HH-RLHF test pairs, seed 42, max_length 384, shared RLTT-local tokenizer, bf16 backbone forward with fp32 captured states/math. Outputs:
- `artifacts/reports/evaluator/hh_layer_states_200_rltt.pt`
- `artifacts/reports/evaluator/hh_layer_states_200_thinking.pt`
- `artifacts/reports/evaluator/probe_bipartite_layers_24_36.json`
- `artifacts/reports/evaluator/probe_cross_backbone_layers.json`

Probe 1 verdict: **contradicts the locked BG tap parameterization**. RLTT layer 24 and layer 36 are fully converged by the v10 threshold rule, while layer 47 remains bipartite and exactly reproduces v10.

| Layer | L1-L4 cos | L2-L4 cos | mean off-diag | min off-diag | Verdict |
|---:|---:|---:|---:|---:|---|
| 24 | +0.9284 | +0.9790 | +0.9632 | +0.9284 | fully converged |
| 36 | +0.9347 | +0.9822 | +0.9682 | +0.9347 | fully converged |
| 47 | +0.7350 | +0.9608 | +0.8849 | +0.7350 | bipartite |

Layer-47 sanity check versus v10 RLTT intrinsic geometry: exact match at reported precision for L1-L4 (+0.7350), L2-L4 (+0.9608), L3-L4 (+0.9961), and L1-L2 (+0.8707).

BG architecture verdict: **BG tap at converged layer should use a newly trained single-state 2048-dim head, not L1/L4 concat. Phase 1 architecture differs from Experiment 2 for that layer.** L4 is a conservative capture/default choice only, not a proven readout winner.

Active spec addendum: `docs/evaluator/bg_tap_interface_revision_2026-05-15.md`.

Revised tap-interface summary:

| Tap | Input interface | Role |
|---:|---|---|
| 24 | single 2048-dim converged loop state | early prune / viability signal |
| 36 | single 2048-dim converged loop state | mid-course consistency / disagreement signal |
| 47 | fused 4096-dim L1/L4 endpoint state | final selector |

The BG program is not rejected. The rejected assumption is uniform L1/L4 fusion at every tap. Phase 1 should train heterogeneous taps: `tap24_single_2048`, `tap36_single_2048`, and `tap47_fused_4096`.

## Converged tap old-head input probe (2026-05-15)

Source probe:
- `utilities/tests/manual/probe_converged_tap_inputs.py`

Inputs: 1000 HH-RLHF test pairs, seed 42, max_length 384, Ouro-RLTT backbone, RLTT-local tokenizer, bf16 backbone forward with fp32 evaluator math. Outputs:
- `artifacts/reports/probes/probe_converged_tap_inputs.json`
- `artifacts/reports/probes/probe_converged_tap_inputs.md`
- `artifacts/reports/probes/probe_converged_tap_inputs_1000.log`

The old published boundary head is not a reliable selector for layers 24/36. Centered accuracy is near chance across all old-head-compatible input constructions, so the apparent L4 top row is not meaningful.

| Layer | Config | centered | canonical | strict | antisym corr | bias/sig |
|---:|---|---:|---:|---:|---:|---:|
| 24 | L4 replicated | 0.512 | 0.763 | 0.570 | +0.905 | 0.761 |
| 36 | L4 replicated | 0.513 | 0.760 | 0.580 | +0.926 | 0.738 |
| 47 | mean replicated | 0.586 | 0.944 | 0.200 | +0.885 | 1.465 |

Interpretation: layer 47 remains the only meaningful old-head-compatible relational readout in this probe. Layers 24/36 are not degenerate: they remain order-sensitive, with high antisymmetry correlation and strict sign-reversal rates around 0.55-0.58. They are just out-of-distribution for the boundary-trained checkpoint on HH centered selection.

Next decision: train small 2048-dim single-state pairwise heads for layers 24/36, comparing at least L1, L4, and mean(L1..L4), with L2 included if time/storage allow. Decide by HH centered diagnostics and generated-branch tournament performance.

Probe 2 verdict: **v9 layer recommendations transfer to RLTT**. Thinking-vs-RLTT cosine is identical at every layer/loop cell, including intermediate layers 24 and 36.

| Layer | loop 1 | loop 2 | loop 3 | loop 4 |
|---:|---:|---:|---:|---:|
| 24 | +1.0000 | +0.9996 | +0.9997 | +0.9997 |
| 36 | +0.9998 | +0.9996 | +0.9997 | +0.9996 |
| 47 | +0.9985 | +0.9993 | +0.9995 | +0.9994 |

Published head sanity on layer-47 boundary states: Pearson(score_T, score_R) = 0.9906, decision agreement = 0.9950, canonical_acc Thinking = 0.9500, canonical_acc RLTT = 0.9450. These reproduce the v10 numbers on the same 200-pair subsample.

## Corrected Math BG-Gate Pilot (2026-05-15)
...
Geometry on math text differs from HH: layers 24/36 remain fully converged, while layer 47 is intermediate/unclear rather than cleanly bipartite.

| Layer | L1-L4 cos | L2-L4 cos | mean off-diag | Verdict |
|---:|---:|---:|---:|---|
| 24 | +0.9133 | +0.9719 | +0.9535 | fully converged |
| 36 | +0.9315 | +0.9718 | +0.9644 | fully converged |
| 47 | +0.6562 | +0.8947 | +0.8504 | intermediate / unclear |

Corrected generated-branch pilot:

| Item | Value |
|---|---:|
| Candidate prompts | 100 |
| Candidate source mix | 50 GSM8K / 50 MATH |
| Kept tournaments | 33 |
| Kept source mix | 26 GSM8K / 7 MATH |
| Generated attempts | 400 |
| Correct / incorrect attempts | 91 / 309 |
| Unparseable attempts | 0 |
| Train / eval tournaments | 25 / 8 |

Held-out eval was tiny but positive: 24/36 trained single-state heads matched or slightly exceeded `47_concat_L1_L4` on pairwise tournament metrics, with no cycles. Treat this as proof-of-possibility, not proof-of-generalization.

Next decision: run a source-stratified, budget-aware gate pass. MATH kept-yield is shaped by generation verbosity and token budget, so log truncation, output token stats, parse failures, and correct/incorrect tournament formation by source before interpreting MATH-heavy failures as tap failures.
