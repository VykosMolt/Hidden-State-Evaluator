...
The current branch-selection frontier has moved past the frozen-backbone steering closure and fixed-composite top4 survival. The active Phase 2a candidate is now the architecture-looped DualAnchor baseline:

- taps: `MIX_CODE_REASONING` + `MIX_OBJECTIVE_ALL`,
- layers: 24, 36, and 47 across loops L1-L4,
...
The current interpretation is that repeated all-loop branching/pruning is viable, earlier-loop L47 should stay active, and recursive perturbation is useful local search. The terminal decision remains the bottleneck; unconditional final top1 is not the locked policy.

June 2 addendum:

`DUALANCHOR_SCIENCE_REASONING_REPAIR_V2_STATUS = REASONING_READY_SCIENCE_DIAGNOSTIC`.
The regenerated science/reasoning repair v2 run locked reasoning terminal survivor-set
handoff as the headline-ready path, but did not clear science as a headline domain.
Science calibration found recipe signal, yet heldout remained
`SCIENCE_BRANCH_GENERATION_STILL_WEAK`; MMLU chemistry/anatomy remain blocked and robust
science parsing remains diagnostic-only. No steering was run.
...
Source scope: this is based on the Markdown documentation under `docs/evaluator`, including the current root docs, `history/`, and `history/raw_archive_2026-05-18/`. The raw archive preserves exact earlier documents; the root docs are the current canonical version.
...
Current canonical docs (post 2026-05-31 consolidation; see `evaluator-navigation-map.md`):
...
- `docs/evaluator/dualanchor-architecture-baseline.md`
- `docs/evaluator/dualanchor-tap-evolution.md`
- `docs/evaluator/branch-generation-and-survival.md`
- `docs/evaluator/terminal-selection-and-arbiters.md`
- `docs/evaluator/domain-transfer-ledger.md`
- `docs/evaluator/evaluator-locus-summary.md`
- `docs/evaluator/flip-test-interpretation.md`
- `docs/evaluator/science-reasoning-repair.md`
- `docs/evaluator/kv-cache-branch-carry.md`
- `docs/evaluator/phase1-controller-and-routing.md`
- `docs/evaluator/steering-and-adapters.md`
- `docs/evaluator/interfaces-and-tools.md`

Older topic docs cited above (controller usage, steering suite/findings, trajectory
sweep, hidden-state/hidden-origin notes, phase1 routing/progress) were archived during
the 2026-05-31 consolidation and now live under `docs/evaluator/history/` (see the
navigation map's Canonical Source Groups for exact paths).
...
- `docs/evaluator/history/raw_archive_2026-05-18/`

## 29. May 31 - Jun 1: Science/Reasoning Pre-Steering Repair

Before committing reasoning or science as a headline steering-comparison domain, two
v1 probes (2026-05-31) and a v2 decision (2026-06-01) tested domain readiness against
the locked architecture-looped DualAnchor baseline.

- Reasoning is headline-ready, but only under the locked terminal handoff (confidence-
  gated top1, else top5/full survivor-set handoff): forced terminal top1 retains only
  0.8750 oracle on 24 reasoning tasks, while the handoff retains 1.0000.
- Science stays diagnostic/excluded. Regenerated source-specific recipes found some
  calibration signal but heldout did not validate a repair; MMLU chemistry/anatomy
  branch generation stayed blocked and the parser patch is diagnostic-only.

Status: `DUALANCHOR_SCIENCE_REASONING_REPAIR_V2_STATUS = REASONING_READY_SCIENCE_DIAGNOSTIC`.
Detail: `science-reasoning-repair.md`. The follow-up MMLU science parser repair v3 is
ongoing; until it completes, science remains diagnostic-only.

## 30. Jun 1 - Jun 4: Generation-Time KV/Cache Branch-Carry and Compute-Saving Splice

A separate mechanical track validated that the architecture-shaped branch loop can run
as real generation-time, branch-specific KV/cache continuation - not just prompt-only
`use_cache=False` perturbation.

- v1 (`autoregressive_kv_branch_carry_v1`) validated the ladder L0-L5 on Ouro's
  UniversalTransformerCache (192 slots, `slot = current_ut*48 + layer_idx`): cached
  decode == full recompute (prefill bit-exact, decode bf16 drift), token-boundary
  forks, batched branches, prune/reorder survivors, current-token and prompt-internal
  perturbation branch caches. Status `PROMPT_INTERNAL_BRANCH_CACHE_VALID`; the partial
  splice was diagnostic-only (no compute saving).
- v2 (`partial_cache_splice_v2`) implemented a true suffix-recompute splice. Because
  the KV cache stores K/V but not the residual stream, the residual hidden at the
  perturbation boundary (loop u, layer L output) is captured during a minimal
  shared-prefix prefill; an additive boundary perturbation is reconstructed without a
  forward, and only the suffix layers are recomputed. The spliced branch cache is
  bit-exact vs a full perturbed-prompt reference. Measured per-branch layer-pass
  savings of 13/38/63/88% for boundary loops 0/1/2/3 (layer 24); amortized across K
  branches (K=2/4/8 -> 32/47/55% at loop 2). Status
  `PARTIAL_CACHE_SPLICE_V2_STATUS = PARTIAL_SPLICE_COMPUTE_SAVING_VALID`.

This is the only place a compute-saving claim is made, and it is a cache mechanism, not
steering or a production routing change. Detail: `kv-cache-branch-carry.md`.

## 31. Final Interpretation

The work has moved through several levels.
...
Fourth, it tested whether that readable signal is already a write/control handle on the frozen backbone. It is not, under the tested methods - inference-time steering is closed under tested safe-alpha methods.

Fifth, it locked an architecture-looped DualAnchor selection baseline and made the pre-steering domain decision: reasoning is headline-ready with terminal handoff, science stays diagnostic.

Sixth, it validated the generation-time substrate the looped branch policy would need: autoregressive branch-specific KV/cache carry, and an amortized compute-saving suffix-recompute splice.

The project is therefore past "can we read anything useful?", past the simple version of "can we steer it at inference time?", and now has both a locked selection baseline and a validated branch-carry/compute-saving cache substrate. The next stage remains Phase 2: training-time integration or a branch-native hidden-origin evaluator that makes the control target explicit, now able to build on a real generation-time branch-carry mechanism.
