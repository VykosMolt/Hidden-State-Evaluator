...
from evaluator2 import ConstitutionalEvaluatorV2, mean_pool, validate_hook_output
...
def evaluate():
    ...
    for idx in range(TRAJECTORY_SAMPLES):
        ...
        with torch.no_grad():
            ...
            sc = evaluator(pooled_chosen).item()
            sr = evaluator(pooled_rejected).item()
    ...
    for start in range(0, len(ds), BATCH_SIZE):
        ...
        with torch.no_grad():
            scores_chosen = evaluator(pooled_chosen)
            scores_rejected = evaluator(pooled_rejected)
        ...
        del pooled_chosen, pooled_rejected, scores_chosen, scores_rejected
