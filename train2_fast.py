...
from evaluator2 import ConstitutionalEvaluatorV2
...
# --- Batching ---
def make_batch(examples, save_mode):
    """
    Collate a list of examples into a batch.
    Handles variable-length padding for raw mode.
    """
    if save_mode == "raw":
        ...
        def pad_and_stack(states_list, masks_list):
            ...
            for s, m in zip(states_list, masks_list):
                ...
                if pad_len > 0:
                    s = F.pad(s, (0, 0, pad_len, 0))  # left-pad
        ...
        raise NotImplementedError("Pooled mode requires evaluator without AttentionPool")
...
def train():
    ...
    print(f"Features: {total_examples} examples, {num_chunks} chunks, mode={save_mode}")
