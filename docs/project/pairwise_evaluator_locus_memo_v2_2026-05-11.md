...
**Code:**
- `utilities/tests/manual/probe_evaluator_hypothesis.py` (v3 — first probe pass, depth sweep + per-loop ablation + flip)
- `utilities/tests/manual/probe_evaluator_v4_ablations.py` (v4 — 26-config ablation table)
- `utilities/tests/manual/probe_evaluator_v4_redo_masked.py` (v4-redo — degeneracy verification + iterated-norm + positional bias)
- `utilities/tests/manual/probe_loop_geometry_hh.py` (v10 — Experiments 1A + 1B, Thinking vs RLTT loop-state geometry on HH text)
**Raw output:**
- `artifacts/reports/probes/probe_evaluator_hypothesis_v3.json` (depth sweep + flip)
- `artifacts/reports/probes/probe_evaluator_v4_ablations.json` (full 26-config table)
- `artifacts/reports/probes/probe_v4_redo.json` (degeneracy verification + iterated-norm sweep)
- `artifacts/reports/probes/probe_loop_geometry_hh.json` (v10 — geometry numbers + four verdicts)
- `artifacts/reports/probes/hh_loop_states_200_thinking.pt` (v10 — captured per-token Thinking states, fp32, 2.5 GB)
- `artifacts/reports/probes/hh_loop_states_200_rltt.pt` (v10 — captured per-token RLTT states, fp32, 2.5 GB)
...
- `utilities/tests/manual/analyze_pairwise_bias_decomposition.py`
- `utilities/tests/manual/probe_hh_norm_mechanism_centered.py`
- `utilities/tests/manual/probe_branch_selection_sim.py`
- `utilities/tests/manual/probe_evaluator_hendrycks_math.py` updated to fall back to `EleutherAI/hendrycks_math/*`
- `artifacts/reports/probes/bias_decomposition_final.json`
- `artifacts/reports/probes/probe_hh_norm_mechanism_centered.json`
- `artifacts/reports/probes/probe_evaluator_hendrycks_math.json`
- `artifacts/reports/probes/probe_branch_selection_sim.json`
...
python utilities/tests/manual/probe_evaluator_hypothesis.py --max-examples 1000

# v4 probe (this memo's results — 26-configuration ablation table):
python utilities/tests/manual/probe_evaluator_v4_ablations.py --max-examples 1000
```
...
```bash
venv/bin/python utilities/tests/manual/analyze_pairwise_bias_decomposition.py \
  artifacts/reports/probes/probe_v5_extended.json \
  artifacts/reports/probes/probe_evaluator_math.json \
  --output-json artifacts/reports/probes/bias_decomposition_all.json \
  --output-md artifacts/reports/probes/bias_decomposition_all.md
```
...
```bash
venv/bin/python utilities/tests/manual/probe_hh_norm_mechanism_centered.py \
  --max-examples 1000 \
  --output-json artifacts/reports/probes/probe_hh_norm_mechanism_centered.json
```
...
```bash
venv/bin/python utilities/tests/manual/probe_evaluator_hendrycks_math.py \
  --max-examples 1000 \
  --output-json artifacts/reports/probes/probe_evaluator_hendrycks_math.json
```
...
```bash
venv/bin/python utilities/tests/manual/probe_branch_selection_sim.py \
  --max-examples 250 \
  --output-json artifacts/reports/probes/probe_branch_selection_sim.json
```
...
```bash
venv/bin/python utilities/tests/manual/probe_all_layers_centered.py \
  --max-examples 1000 \
  --score-chunk-size 32 \
  --output-json artifacts/reports/probes/probe_all_layers_centered.json
```

Analysis:

```bash
venv/bin/python utilities/tests/manual/analyze_all_layer_taps.py \
  --input-json artifacts/reports/probes/probe_all_layers_centered.json \
  --output-json artifacts/reports/probes/probe_all_layers_analysis.json \
  --output-md artifacts/reports/probes/probe_all_layers_analysis.md
```
...
```bash
venv/bin/python utilities/tests/manual/probe_layer_tap_math_actual.py \
  --pair-examples 1000 \
  --branch-examples 250 \
  --output-json artifacts/reports/probes/probe_layer_tap_math_actual.json
```
...
```bash
venv/bin/python utilities/tests/manual/probe_layer_tap_cached_domains.py \
  --max-examples-per-dataset 120 \
  --output-json artifacts/reports/probes/probe_layer_tap_cached_domains.json
```
...
```bash
venv/bin/python utilities/tests/manual/probe_multitap_ensemble.py \
  --max-examples-per-dataset 120 \
  --include-math \
  --math-examples 250 \
  --layers 20,24,36,40,45,46,47 \
  --output-json artifacts/reports/probes/probe_multitap_ensemble.json
```
...
**Source script:** `utilities/tests/manual/probe_loop_geometry_hh.py`
...
**Head:** `artifacts/checkpoints/pairwise/pairwise_epoch2.pt` — the published 5M-param GRU.
**Outputs:**
- `artifacts/reports/probes/probe_loop_geometry_hh.json` — full numbers
- `artifacts/reports/probes/hh_loop_states_200_thinking.pt` — per-token states + masks (Thinking, fp32, 2.5 GB)
- `artifacts/reports/probes/hh_loop_states_200_rltt.pt` — same, RLTT (2.5 GB)
...
**Script:** `utilities/tests/manual/probe_l1_ablation.py`
**Raw output:** `artifacts/reports/probes/probe_l1_ablation.json`
